...
def main(args):
    ...
    if args.task in ("blip2-image_captioning", "blip2-vqav2", "blip2-gqa"):
        ...
        quantizer = BLIP2Quantizer(model, processor, gpu, config)
        ...
        quantizer = LlavaQuantizer(model, processor, gpu, config)
    else:
        raise ValueError("Unsupported task")
    ...
    elif args.task == "blip2-vqav2" or args.task == "blip2-gqa":
    ...
    # Quantize the model
    if "blip2" in args.task:
        quantizer.quantize(dataset, args.task)
    else:
        quantizer.quantize(dataset)
    ...
    if "image_text_retrieval" not in args.task:
        quantizer.prepare_for_inference()
        model = quantizer.model
        #quantizer.prepare_for_inference(model, gpu)
    #BLIP2Quantizer.prepare_for_inference(model, gpu)
