...
parser.add_argument('--offset', type=int, required=True, help='Offset for the job array')
parser.add_argument('--batchsize', type=int, required=True, help='Batch size for the job array')
...
# Create SLURM array script with the given offset and batch size
slurm_script_content = f"""#!/bin/bash

#SBATCH --job-name=batch_run
#SBATCH --output={args.output_dir}/slurm-%A_%a.out
#SBATCH --error={args.output_dir}/slurm-%A_%a.err
#SBATCH --array=0-{args.batchsize-1} # Job array size
#SBATCH --time=8:00:00
#SBATCH --requeue
...
echo "Offset: {args.offset}"

# Calculate the actual config index based on the task ID
CONFIG_INDEX=$((SLURM_ARRAY_TASK_ID + {args.offset}))
...
slurm_script_path = 'submit_array.sh'
...
print(f"Submitted {args.batchsize} jobs starting from offset {args.offset}")
