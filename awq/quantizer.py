...
class BaseAWQQuantizer:
    ...
    def quantize(self):
        ...
        first_inputs, self.layer_args, self.layer_kwargs = self._gather_first_inputs(
        ...
        for layer_group, modules in layer_groups.items():
            ...
            for i in tqdm(range(len(modules)), desc=f"Quantizing {layer_group}"):
                ...
                named_linears, w_bits_dict = self._filter_named_linears(
                    named_linears, layer_group
                )
                # gather inputs to each nn.Linear via pytorch hooks
                # NOTE: also setting self.inps for next iteration
                linear_inputs = self._gather_linear_inputs(
                    layer, named_linears, layer_group
                )
                ...
                grouped_mods = self._group_modules_for_scaling(
                    layer, linear_inputs, layer_group
                )

                # compute scales over each group of modules (weight blocks) to quantize
                scales = [
                    self._compute_scales(layer, **group) for group in grouped_mods
                ]
                ...
                clips = self._search_best_clip(
                    named_linears, linear_inputs, w_bits_dict
                )
    ...
    def _gather_linear_inputs(self, layer, named_linears, layer_group):
        ...
        for name, mod in named_linears.items():
            hooks.append(
                mod.register_forward_hook(
                    partial(input_hook, module_name=name, inputs=inputs)
                )
            )
        ...
        out = layer(
            self.inps, *self.layer_args[layer_group], **self.layer_kwargs[layer_group]
        )[0].to("cpu")
    ...
    def _compute_scales(
        self, layer, prev_op, modules, inp, parent_module, layer_kwargs, w_bits
    ):
        """
        ...
        for ratio in range(n_grid):
            ...
            for mod in modules:
                ...
                mod.weight.data = (
                    self.pseudo_quantize_tensor(mod.weight.data, w_bits)[0]
                    / scales_view
                )
    ...
    def _filter_named_linears(self, named_linears, layer_group):
        ...
        valid_modules = [
            self.group2modules[layer_group][k] for k in self.config[layer_group]
        ]
                ...
                if "cross_attn" in self.config[layer_group]:
                    ...
                    grouped_mods.append(
                        dict(
                            ...
                            modules=[layer.crossattention.attention.value],
...
class Blip2ForConditionalGenerationAWQQuantizer(Blip2AWQQuantizer):
    def __init__(self, model, device, inputs_processor, dataset, config, processor_kwargs=None):
    ...
    def _get_model_layer_groups(self):
        ...
        if "vit_layers" in self.config:
            ...
            self.excluded_mods.extend(
                get_mods(self.model.vision_model, non_linears_only=True)
            )
        else:
            self.excluded_mods.extend(
                get_mods(self.model.vision_model, non_linears_only=False)
            )

        if "qformer_layers" in self.config:
            layer_groups["qformer_layers"] = self.model.qformer.encoder.layer
            self.excluded_mods.extend(
                get_mods(self.model.qformer, non_linears_only=True)
            )
        else:
            self.excluded_mods.extend(
                get_mods(self.model.qformer, non_linears_only=False)
            )

        if "llm_layers" in self.config:
            layer_groups["llm_layers"] = self.model.language_model.model.decoder.layers
            self.excluded_mods.extend(
                get_mods(self.model.language_model, non_linears_only=True)
            )
            # exclude this final projection still
            self.excluded_mods.append(self.model.language_model.lm_head)
        else:
            self.excluded_mods.extend(
                (get_mods(self.model.language_model, non_linears_only=False))
            )

        return layer_groups

    def _prepare_input(self, inp):
        images = [x["images"] for x in inp] if "images" in inp[0] else None
        text = [x["text_input"] for x in inp] if "text_input" in inp[0] else None
        if text is None:
            X = self.inputs_processor(
                images=images,
                return_tensors="pt",
            ).to(self.device)
        else:
    ...
    def _get_calibration_set(self):
        ...
        calibration_set = get_calibration_set(self.dataset, self.n_samples)
        return self._prepare_input(calibration_set)

    def _run_model(self, calibration_set):
        if isinstance(calibration_set, dict):
            self.model(**calibration_set)
        else:
            self.model.generate(**calibration_set)
