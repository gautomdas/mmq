...
class LlavaAWQQuantizer(BaseAWQQuantizer):
    def __init__(
        self, model, device, inputs_processor, dataset, config, dataset_name="VQAv2"
    ):
    ...
    def _get_model_layer_groups(self):
        ...
        if quant_vision_flag:
            layer_groups["vision_layers"] = (
                self.model.vision_tower.vision_model.encoder.layers
            )

        self.excluded_mods.extend(
            get_mods(self.model.vision_tower, non_linears_only=quant_vision_flag)
        )
        ...
        self.excluded_mods.extend(
            get_mods(self.model.language_model, non_linears_only=quant_llm_flag)
        )
            ...
            if "self_attn" in self.config[layer_group]:
                ...
                grouped_mods.append(
                    dict(
                        ...
                        modules=[layer.self_attn.q_proj],
                        inp=linear_inputs["self_attn.q_proj"],
                ...
                grouped_mods.append(
                    dict(
                        ...
                        modules=[layer.self_attn.k_proj],
                ...
                grouped_mods.append(
                    dict(
                        ...
                        modules=[layer.self_attn.v_proj],
                ...
                if (
                    layer.self_attn.v_proj.weight.shape
                    == layer.self_attn.o_proj.weight.shape
                ):
                    grouped_mods.append(
                        dict(
                            prev_op=layer.self_attn.v_proj,
                            modules=[layer.self_attn.o_proj],
    ...
    def _get_calibration_set(self):
        ...
        samples = self.inputs_processor(
            images=imgs, text=prompts, return_tensors="pt", padding=True
        ).to(self.device)
