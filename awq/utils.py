...
def get_named_linears(module):
    return {
        name: mod for name, mod in module.named_modules() if isinstance(mod, nn.Linear)
    }
    ...
    if non_linears_only:
        return (
            ...
            if len(children) == 0
                ...
                ci
                for c in children
                for ci in get_mods(c, non_linears_only)
                if type(ci) != torch.nn.Linear
            ]
        )
    else:
        return (
            [model]
            if len(children) == 0
            else [ci for c in children for ci in get_mods(c, non_linears_only)]
        )
